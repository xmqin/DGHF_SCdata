...
printf "%16s %16s %16s %16s %16s  %16s  %16s  %16s  \n" "Natom" "Ncore" "TimeALB " "TimeDG" "TimeDGHFX " " TimeDIAG" "TimeDensity" "TimeSCF"

#for natom in 6480 12960 25920  51840 103680
for natom in 180 720 1620 2880 4500 6480 8820 11520 25920 46080 72000 103680
# LiNa1254400  LiNa156800  LiNa2508800  LiNa2508800_1  LiNa2508800_2  LiNa313600  LiNa627200
do
cd G$natom
ncore=`grep "bsub" submit.sh | awk '{printf "%s \n", $13}'`
#node=`echo $ncore/6 |bc`
ne=`grep "sum of Pseudocharge" statfile.0 | tail -1| awk '{printf "%d \n", $7}'`
na=`echo $ne/4 |bc`
nele=`echo $natom*6 |bc`
...
printf "%16s  %16s  %16s  %16s  %16s  %16s  %16s  %16s \n" "$na" "$ncore " " $time_ALB" " $time_DG" " $time_DGHFX " " $time_Diag " "$time_Density"  "$time_SCF" #"$time_symfac"  "$time_pole"
#printf "\n"
#printf "|$marker $marker1 $marker2|\n"
