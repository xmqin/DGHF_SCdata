...
printf "%16s %16s  %16s  %16s  %16s  %16s  %16s  %16s \n" "Natom" "Nelect" "TimeALB " "TimeDG" "TimeDGHFX " " TimeDIAG" "TimeDensity" "TimeSCF"

#for natom in 6480 12960 25920  51840 103680
for natom in  LiNa156800   LiNa313600  LiNa627200 LiNa1254400 LiNa2508800
...
na=`grep "sum of Pseudocharge" statfile.0 | tail -1| awk '{printf "%d \n", $7}'`
nele=`echo $na*7 |bc`
...
printf "%16s  %16s  %16s  %16s  %16s  %16s  %16s  %16s \n" "$na" "$nele"  " $time_ALB" " $time_DG" " $time_DGHFX " " $time_Diag " "$time_Density"  "$time_SCF" #"$time_symfac"  "$time_pole"
#printf "\n"
#printf "|$marker $marker1 $marker2|\n"
